            ...
            public void onClick(View view) {
                if(!TextUtils.isEmpty(phone.getText().toString().trim())){
                    if(phone.getText().toString().trim().length() == 11){
                        ...
                        Toast.makeText(DuanXinActivity.this, "请输入完整电话号码", Toast.LENGTH_LONG).show();
                        phone.requestFocus();
                    }
                }else{
                    Toast.makeText(DuanXinActivity.this, "请输入您的电话号码", Toast.LENGTH_LONG).show();
                    phone.requestFocus();
                }
            }
        });
            ...
            public void onClick(View view) {
                if(!TextUtils.isEmpty(cord.getText().toString().trim())){
                    if(cord.getText().toString().trim().length()==4){
                        ...
                        Toast.makeText(DuanXinActivity.this, "请输入完整验证码", Toast.LENGTH_LONG).show();
                        cord.requestFocus();
                    }
                }else{
                    Toast.makeText(DuanXinActivity.this, "请输入验证码", Toast.LENGTH_LONG).show();
                    cord.requestFocus();
                }
            }
        });
        ...
        public void handleMessage(Message msg) {
            ...
            if (result == SMSSDK.RESULT_COMPLETE) {
                ...
                if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) {//提交验证码成功,验证通过
                    ...
                    intent.putExtra("return_yanzhengma", "（已核验）");
                ...
                if(flag){
                    getCord.setVisibility(View.VISIBLE);
                    Toast.makeText(DuanXinActivity.this, "验证码获取失败，请重新获取", Toast.LENGTH_SHORT).show();
                    phone.requestFocus();
                }else{
                    ((Throwable) data).printStackTrace();
                    int resId = getStringRes(DuanXinActivity.this, "smssdk_network_error");
                    Toast.makeText(DuanXinActivity.this, "验证码错误", Toast.LENGTH_SHORT).show();
                    cord.selectAll();
                    if (resId > 0) {
                        Toast.makeText(DuanXinActivity.this, resId, Toast.LENGTH_SHORT).show();
                    }
                }
            }
        }
